class SoundcloudPlayerPlugin extends ListiaryPlugin {
	...
	testModeListIds = [
		...
		"radiowatch.playlists.bg.energy"
	];
	...
	AddPlayer() {
		...
		var html = "<iframe id='soundcloud-player' style='margin-top: 30px; margin-bottom: 30px; max-width: 12cm; width: 100%; height: 166px; border-radius: 10px 10px 0 0; box-shadow: 0 0 30px rgba(0, 255, 255, 0.3); display: block;'scrolling='no' frameborder='no' allow='autoplay' src='https://w.soundcloud.com/player/?url='></iframe>";
		...
		{	
			...
			spans.forEach(span => 
			{
				...
				span.onclick = (event) => 
				{
					...
					let iframeElement = document.getElementById('soundcloud-player');
