const mainTree = {
	...
	availableWidth: 0,
	isMobile: false,
	overlapType: "dots",				//"dots" | "multyline" | "basic"
	...
	// </li>
		...
		if(item.type == "empty") {
			...
			if (item.type == "comment") {
				...
				"<span style='" + spanStyle + "'>" + itext + "</span>"
				links +
				"</li>";
			}
			else if (item.type == "nlcomment") {
	...
	translateLinks: function(item) {
		...
		if(mainTree.overlapType == "basic") return mainTree.translateLinks_Basic(item);
	...
	//Choose from the different length versions, if available
