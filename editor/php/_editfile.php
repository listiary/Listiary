<?php
	...
	try
	{
		...
		$newArticle = getNewArticle($curArticle);
	...
	function getNewArticle($oldArticle) {
		...
		if($newArticle == $oldArticle) exit("Articles are the same, nothing to edit.");	
	...
	function updateDescribeDocument($conn, $articleName, $newArticleText) {
		...
		//update describe Document
		$entry_safe = mysqli_real_escape_string($conn, $articleName);
		$content_safe = mysqli_real_escape_string($conn, $newArticleText);
