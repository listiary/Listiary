const mainTree = {
	...
	Draw: function(id){
		...
		if (mainTree.isMobile) mainTree.draw(id);
	...
	drawBreadcrumb_Compacted: function(id) {
		...
		while (cur.parentItem != undefined) {
			...
			else
			{
				...
				breadCrumb = "<a class=\"breadcrumbmain\" style='display: none;' href=\"javascript:mainTree.Draw(\'" + 
					cur.id + "\');\">" + cur.text + "</a>" + breadCrumb;
			}
		}
		if (breadCrumb.length == 0)
		...
		else if (isfirst == 0)
		{
			...
			breadCrumb = "<span id='breadcrumbmain_expander' style='vertical-align: 0px; font-size:0.5em; background: black; color: white; border-radius: 25px; padding-left: 4px; padding-right: 4px; font-weight: 700; cursor: pointer;' onclick='mainTree.expandBreadcrumb();'>•••</span>" + breadCrumb;
		}
	...
	_makeBranchItem_Basic: function(id, text, linksHtml, availableWidth) {
		...
		return "<li class='treedata' style='" +
				...
				"<a class=\"breadcrumb\" href=\"javascript:mainTree.Draw(\'" + id + "\');\">" +
				text + " " + "</a>" + linksHtml + "</li>";
	},
	...
	translateItem_Link: function(item) {
		...
		if(item.type == "empty") {
			...
			if (item.type == "comment") 
			{
				return mainTree._makeItem_Link_Comment(cssFromDecorators, links, itext);
			}
			else if (item.type == "nlcomment") 
			{
				return mainTree._makeItem_Link_NlComment(cssFromDecorators, links, itext);
			}
			else if (item.color != undefined) 
			{
				return mainTree._makeItem_Link_Colored(cssFromDecorators, links, itext, item.color);
			}
			else //item.type == "item"
			{ 
				return mainTree._makeItem_Link(cssFromDecorators, links, itext);
			}
		}
	},
	_makeItem_Link_Empty() {
		
		return "<li>&nbsp;</li>";
	},
	_makeItem_Link(cssFromDecorators, linkHtml, text) {
	...
	_makeItem_Link_Colored(cssFromDecorators, linkHtml, text, color) {
	...
	_makeItem_Link_Comment(cssFromDecorators, linkHtml, text) {
	...
	_makeItem_Link_NlComment(cssFromDecorators, linkHtml, text) {
	...
	translateLinks_One: function(item) {
		...
		for (var i = 0; i < item.links.length; i++) {
			...
			links += "<a target='_blank' style='all: unset; cursor: pointer;' href='" +
			item.links[i].url +
			"'>~~~</a>";
			break;
		}
		...
		if(links == "")
		{
			links = "<a style='all: unset;' href='javascript:void(0);'>~~~</a>";
		}
		return links;
	},
